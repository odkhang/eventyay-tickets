...
class OrganizerUpdate(OrganizerPermissionRequiredMixin, UpdateView):
    ...
    def form_valid(self, form):
        ...
        if self.footer_links_formset.has_changed():
            self.request.organizer.log_action(
                "eventyay.organizer.footerlinks.changed",
                user=self.request.user,
                data={"data": self.footer_links_formset.cleaned_data},
            )
        if form.has_changed():
            self.request.organizer.log_action(
...
class OrganizerMailSettings(OrganizerSettingsFormView):
    ...
    def get_success_url(self):
        return reverse(
            "control:organizer.settings.mail",
            kwargs={
                "organizer": self.request.organizer.slug,
            },
        )
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            if request.POST.get("test", "0").strip() == "1":
                backend = self.request.organizer.get_mail_backend(
